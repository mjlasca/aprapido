...
namespace ProyectoBrokerDelPuerto
{
    class lineas_propuestas_aux
    {
        ...
        public lineas_propuestas_aux(bool inst = false)
        {
            if(inst)
                this.install();
        }
        ...
        {
            //sql = "DROP TABLE lineas_propuestas_aux";
            //con.query(sql);
            if (MDIParent1.baseDatos == "MySql")
            {
                sql = "CREATE TABLE if not exists lineas_propuestas_aux (id serial PRIMARY KEY,  id_propuesta INT(15) NULL, documento VARCHAR(100) NULL, tipo_documento VARCHAR(100) NULL, apellidos VARCHAR(200) NULL , nombres VARCHAR(200) NULL , fecha_nacimiento DATE NULL,  " +
            ...
            {
                sql = "CREATE TABLE if not exists lineas_propuestas_aux (id INTEGER PRIMARY KEY,  id_propuesta INT(15) NULL, documento VARCHAR(100) NULL, tipo_documento VARCHAR(100) NULL, apellidos VARCHAR(200) NULL , nombres VARCHAR(200) NULL , fecha_nacimiento DATE NULL,  " +
                "id_actividad VARCHAR(100) NULL,id_clasificacion VARCHAR(100) NULL, premio DOUBLE NULL, ultmod DATETIME NULL, user_edit VARCHAR(100) NULL, codestado VARCHAR(2) NULL );";
            }
            ...
            if (MDIParent1.baseDatos == "MySql")
            {
                ...
                string sql1 = "SELECT COUNT(1) as cant FROM information_schema.statistics WHERE TABLE_SCHEMA = DATABASE()   AND TABLE_NAME = 'lineas_propuestas_aux'  AND INDEX_NAME = 'idx.id_propuesta'; ";
                DataSet ds = con.query(sql1);
                if (ds.Tables[0].Rows[0]["cant"].ToString() != "1")
                {
                    sql1 = "CREATE INDEX `idx.id_propuesta` ON `lineas_propuestas_aux` (`id_propuesta`);";
                    con.query(sql1);
                }
                sql1 = "SELECT COUNT(1) as cant FROM information_schema.statistics WHERE TABLE_SCHEMA = DATABASE()   AND TABLE_NAME = 'lineas_propuestas_aux'  AND INDEX_NAME = 'idx.prefijo'; ";
                ds = con.query(sql1);
                if (ds.Tables[0].Rows[0]["cant"].ToString() != "1")
                {
                    sql1 = "CREATE INDEX `idx.prefijo` ON `lineas_propuestas_aux` (`prefijo`);";
                    con.query(sql1);
                }
                sql1 = "SELECT COUNT(1) as cant FROM information_schema.statistics WHERE TABLE_SCHEMA = DATABASE()   AND TABLE_NAME = 'lineas_propuestas_aux'  AND INDEX_NAME = 'idx.idprefijo'; ";
                ds = con.query(sql1);
                if (ds.Tables[0].Rows[0]["cant"].ToString() != "1")
                {
                    sql1 = "CREATE INDEX `idx.idprefijo` ON `lineas_propuestas_aux` (`idprefijo`);";
                    con.query(sql1);
                }

            }

            con.query("UPDATE lineas_propuestas_aux SET codempresa = '" + MDIParent1.codempresa + "' WHERE (codempresa IS NULL OR codempresa = '')  AND ultmod > '" + DateTime.Now.AddMonths(-3).ToString("yyyy-MM-dd HH:mm:ss") + "' ");
        ...
        private void addColumn()
        {
            if (MDIParent1.baseDatos == "MySql")
            {
                try
                {
                    ...
                    if (con.query("SHOW COLUMNS FROM lineas_propuestas_aux WHERE Field = 'prefijo' ").Tables[0].Rows.Count == 0)
                        con.query("ALTER TABLE lineas_propuestas_aux ADD COLUMN prefijo VARCHAR(50);");
                    if (con.query("SHOW COLUMNS FROM lineas_propuestas_aux WHERE Field = 'actividad' ").Tables[0].Rows.Count == 0)
                        con.query("ALTER TABLE lineas_propuestas_aux ADD COLUMN actividad VARCHAR(300);");
                    if (con.query("SHOW COLUMNS FROM lineas_propuestas_aux WHERE Field = 'clasificacion' ").Tables[0].Rows.Count == 0)
                        con.query("ALTER TABLE lineas_propuestas_aux ADD COLUMN clasificacion VARCHAR(300);");
                    if (con.query("SHOW COLUMNS FROM lineas_propuestas_aux WHERE Field = 'fechaDesde' ").Tables[0].Rows.Count == 0)
                        con.query("ALTER TABLE lineas_propuestas_aux ADD COLUMN fechaDesde DATETIME NULL;");
                    if (con.query("SHOW COLUMNS FROM lineas_propuestas_aux WHERE Field = 'fechaHasta' ").Tables[0].Rows.Count == 0)
                        con.query("ALTER TABLE lineas_propuestas_aux ADD COLUMN fechaHasta DATETIME NULL;");
                    if (con.query("SHOW COLUMNS FROM lineas_propuestas_aux WHERE Field = 'idprefijo' ").Tables[0].Rows.Count == 0)
                        con.query("ALTER TABLE lineas_propuestas_aux ADD COLUMN idprefijo DOUBLE NOT NULL;");
                    if (con.query("SHOW COLUMNS FROM lineas_propuestas_aux WHERE Field = 'codempresa' ").Tables[0].Rows.Count == 0)
                        con.query("ALTER TABLE lineas_propuestas_aux ADD COLUMN codempresa VARCHAR(150) NULL;");
                }
                catch
                {
                    //
            ...
            else
            {
                ...
                con.query("ALTER TABLE lineas_propuestas_aux ADD COLUMN fechaDesde DATETIME NULL;");
                con.query("ALTER TABLE lineas_propuestas_aux ADD COLUMN fechaHasta DATETIME NULL;");
                con.query("ALTER TABLE lineas_propuestas_aux ADD COLUMN idprefijo DOUBLE  NULL;");
                con.query("ALTER TABLE lineas_propuestas_aux ADD COLUMN codempresa VARCHAR(150) NULL;");
            }
        ...
        public DataSet get_idpropuesta(string idpropuesta_, string prefijo_ )
        {
            ...
            lineas_propuestas_aux lin = new lineas_propuestas_aux();
            sql = "SELECT *,(SELECT cod  FROM actividades t1 WHERE t1.id = t2.id_actividad) as cod_actividad, (SELECT cod  FROM clasificaciones t1 WHERE t1.id = t2.id_clasificacion) as cod_clasificacion, (SELECT SUM(t1.prima)  FROM propuestas t1 WHERE t1.prefijo = t2.prefijo AND t1.idpropuesta = t2.id_propuesta) as prima, " +
                ...
                " (SELECT SUM(t1.nota) FROM propuestas t1 WHERE t1.prefijo = t2.prefijo AND t1.idpropuesta = t2.id_propuesta) as nota FROM lineas_propuestas_aux t2 WHERE t2.idprefijo = '" +
            ...
            {
                Console.Write("ERROR al consultar lineas_propuestas_aux");
            }
        ...
        public DataSet get_entre_controlventas_efectivo(string fecha1, string fecha2, string refe = "")
        {
            ...
            if (MDIParent1.baseDatos == "SQlite")
            {
                sql = "SELECT t0.id,t0.idpropuesta,t0.prima,t0.prefijo,t0.referencia,t0.tipopago,t0.fecha_paga,t0.compformapago,t0.formadepago,t0.premio_total,t0.documento,t0.fechaDesde,t0.fechaHasta,t0.paga, DATE(t0.ultmod) as ultmod, t0.meses," +
                     ...
                     " FROM propuestas t0 INNER JOIN  lineas_propuestas_aux t3 ON " +
        ...
        public DataSet get_entrefechaslineas(string fec1, string fec2, string referencia_)
        {
            ...
            if (MDIParent1.baseDatos == "MySql")
            {
                ...
                " FROM lineas_propuestas_aux t1 INNER JOIN propuestas t2 ON t1.prefijo = t2.prefijo AND t2.idpropuesta = t1.id_propuesta"+
                " WHERE " +
        ...
        public DataSet get(string id_)
        {
            ...
            sql = "SELECT * FROM lineas_propuestas_aux WHERE codestado = 1 AND id= '" + id_ + "' ";
            try
            {
                ds = con.query(sql);
            }
        ...
        public DataSet get_idprefijo(string idPropuesta_, string prefijo_)
        {
            ...
            sql = "SELECT * FROM lineas_propuestas_aux WHERE  id_propuesta= '" + idPropuesta_ + "' AND prefijo = '" + prefijo_ + "' ";
            try
            {
        ...
        public DataSet get_all_findia(string fecha_)
        {
            ...
            sql = "SELECT *, "+
                ...
                " FROM lineas_propuestas_aux WHERE DATE(ultmod) = '" + fecha_ + "'  ";
            try
            {
                ds = con.query(sql);
            }
        ...
        {
            DataSet ds = new DataSet();
            sql = "SELECT * FROM lineas_propuestas_aux WHERE DATE(ultmod) >= '"+fecha+"' AND DATE(ultmod) <= '" + DateTime.Now.ToString("yyyy-MM-dd") + "' ";
            try
            {
                ds = con.query(sql);
            }
        ...
        public DataSet get_all()
        {
            ...
            sql = "SELECT * FROM lineas_propuestas_aux WHERE codestado = 1";
            try
            {
                ds = con.query(sql);
            }
        ...
        public DataSet get_all_busqueda(string coincidencia)
        {
            ...
            sql = "SELECT * FROM lineas_propuestas_aux WHERE codestado = 1 AND " +
        ...
        {
            sql = "DELETE FROM lineas_propuestas_aux";
            con.query(sql);
        }

        private bool exist()
        {
            sql = "SEECT COUNT(1) as cant FROM lineas_propuestas_aux WHERE  prefijo = '" + this.prefijo + "' AND id_propuesta = '" + this.id_propuesta + "'";
        ...
        public bool save_nube()
        {
            try
            {
                if (!this.exist())
                {
                    sql = "INSERT INTO lineas_propuestas_aux (" + this.columns + ") VALUES(" +
                   "'" + this.id_propuesta + "'," +
                   "'" + this.documento + "'," +
                   "'" + this.tipo_documento + "'," +
                   "'" + this.apellidos + "'," +
        ...
        {
            con.query("INSERT INTO lineas_propuestas_aux (" + this.columns + ") VALUES " + sql1);
        }
        ...
        {
            sql = "DELETE FROM lineas_propuestas_aux WHERE idprefijo = '" + idprefijo_ + "' AND prefijo = '"+prefijo_+"' ";
            con.query(sql);
        }

        public void delete_concat(string sqlconcat)
        {
            sql = "DELETE FROM lineas_propuestas_aux WHERE "+ sqlconcat;
            con.query(sql);
        }

        public void delete()
        {
            sql = "UPDATE lineas_propuestas_aux SET codestado = 0 WHERE id = '" + this.id + "' ";
            con.query(sql);
        }

        public static implicit operator lineas_propuestas_aux(lineas_propuestas v)
        {
