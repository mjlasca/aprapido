        ...
        public async Task<bool> Post(List<clasificaciones> lsclasificaciones)
        {
            ...
            try
            {
                ...
                else
                {
                    rest = false;
                    logs log = new logs();
                    log.newError(@"POST" + this.path + "/setclasificaciones/" + MDIParent1.codempresa, "No se ha podido enviar los datos" + response.StatusCode);
                }
            }
            catch (Exception ex)
            {
                rest = false;
                logs log = new logs();
                log.newError(@"POST" + this.path + "/setclasificaciones/" + MDIParent1.codempresa, "Ha ocurrido un error al obtener los datos " + ex.Message);
            }
