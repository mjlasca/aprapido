...
namespace ProyectoBrokerDelPuerto
{
    class ApiActividades
    {
        ...
        public ApiActividades()
        {
            ...
            this.path = "/api/actividades";
            this.repen = new RegisterPending();
        }


        public async Task<List<actividades>> Get()
        {
            List<actividades> ls = new List<actividades>();
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    var result = JsonConvert.DeserializeObject<List<actividades>>(jsonContent);
                    ls = result;
                }
                else
                {
        ...
        public async Task<bool> Post(List<actividades> lsactividades)
        {
            bool rest = false;

            var jsonData = JsonConvert.SerializeObject(lsactividades);
            var client = new HttpClient();

            client.BaseAddress = new Uri(this.baseEndPoint);
            var request = new HttpRequestMessage
            {
                RequestUri = new Uri(this.path + "/setactividades/" + MDIParent1.codempresa, UriKind.Relative),
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    this.repen.path = this.path + "/setactividades/" + MDIParent1.codempresa;
                ...
                else
                {
                    rest = false;
                    logs log = new logs();
                    log.newError(@"POST" + this.path + "/setactividades/" + MDIParent1.codempresa, "No se ha podido enviar los datos" + response.StatusCode);
                }
            }
            catch (Exception ex)
            {
                rest = false;
                logs log = new logs();
                log.newError(@"POST" + this.path + "/setactividades/" + MDIParent1.codempresa, "Ha ocurrido un error al obtener los datos " + ex.Message);
            }
